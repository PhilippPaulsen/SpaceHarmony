...
function _generateMetaData(form, options, validationResults) {
    ...
    return {
        ...
        "source": "Automatischer Generator v1.4",
...
function exportAsObj(form) {
    let objContent = "# Generated by SpaceHarmony Form Generator v1.4\n";
...
function generateMultipleForms(config = {}) {
    if (!fs || !path) {
    ...
    const filter = (form) => form.metadata.faces >= minFaces;
    ...
    const successRate = (savedCount / count * 100).toFixed(1);
    console.log(`Trefferquote: ${savedCount} von ${count} Formen erfüllt Kriterien (≈ ${successRate}%).`);
...
if (typeof require !== 'undefined' && require.main === module) {
    ...
    generateMultipleForms({
        count: 300,
        minFaces: 0, // 0 = alle Formen speichern, 1 = nur die mit Flächen
        gridSize: 3,
        pointDensity: 2,
        generationOptions: { 
            singleStroke: true, 
            minSteps: 5, // Mindestlänge des Linienzugs
            maxSteps: 25 
        },
        saveJson: true,
        saveObj: true,
